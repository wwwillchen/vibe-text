...
const toneToY: Record<Tone, number> = { 'Professional': 25, 'Casual': 75 };
const lengthToX: Record<Length, number> = { 'Shorter': 25, 'Longer': 75 };
...
const ToneLengthSelector: React.FC<ToneLengthSelectorProps> = ({ selectedTone, selectedLength, onSelect }) => {
  ...
  const mapPositionToValues = (xPercent: number, yPercent: number): { tone: Tone; length: Length } => {
    let tone: Tone = yPercent < 50 ? 'Professional' : 'Casual';
    let length: Length = xPercent < 50 ? 'Shorter' : 'Longer';
    return { tone, length };
  };
  ...
  const tones: Tone[] = ['Casual', 'Professional'];
  const lengths: Length[] = ['Shorter', 'Longer'];
